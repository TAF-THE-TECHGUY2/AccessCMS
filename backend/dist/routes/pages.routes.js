...
const LOCKED_PASSTHROUGH = ["lockedTitle", "lockedSubtitle"];
/**
 ...
 */
const applyMemberAccess = (page, isMember) => {
    ...
    return {
        ...
        sections: plain.sections.map((section) => {
            if (section?.access !== "MEMBERS" || isMember)
                return section;
            const data = {};
            for (const key of LOCKED_PASSTHROUGH) {
                if (section?.data?.[key] !== undefined)
                    data[key] = section.data[key];
            }
...
adminRouter.post("/", validate(pageSchema), async (req, res) => {
...
adminRouter.patch("/:id", validate(pageSchema.partial()), async (req, res) => {
...
adminRouter.post("/:id/publish", async (req, res) => {
    const page = await Page.findByIdAndUpdate(req.params.id, { status: "published", publishedAt: new Date() }, { new: true });
    if (!page)
        return res.status(404).json({ message: "Page not found" });
