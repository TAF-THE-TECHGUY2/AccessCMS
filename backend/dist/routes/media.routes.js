...
router.patch("/:id/access", async (req, res) => {
    ...
    if (access === "MEMBERS" && env.storageDriver === "s3") {
        return res.status(409).json({
            message: "Members-only files require STORAGE_DRIVER=local. On S3 the file is served directly by the bucket and cannot be gated by this API.",
        });
    }
    const media = await Media.findByIdAndUpdate(req.params.id, { access }, { new: true });
    if (!media)
