...
publicRouter.post("/", submitLimiter, validate(contactSchema), async (req, res) => {
    const { name, email, subject, phone, topic, message } = req.body;
...
adminRouter.get("/", async (req, res) => {
    ...
    const filter = status && ["new", "read", "archived"].includes(status) ? { status } : {};
...
adminRouter.patch("/:id", validate(statusSchema), async (req, res) => {
