...
import type { MemberRequest } from "./member.js";
...
const TTL_MS = 60_000;
const cache = new Map<string, { access: string; at: number }>();

export const invalidateUploadAccess = (key?: string) => {
  if (key) cache.delete(key);
  else cache.clear();
};

const lookupAccess = async (key: string): Promise<string> => {
...
export const guardUploads = async (
  req: MemberRequest,
  res: Response,
  next: NextFunction
) => {
