...
export const resolveMember = (
  ...
  try {
    const claims = jwt.verify(token, env.memberJwtSecret) as MemberClaims;
    if (claims?.sub) req.member = claims;
  } catch {
...
export const memberFirstName = (member: MemberClaims | null | undefined) =>
  member?.name ? String(member.name).trim().split(/\s+/)[0] : undefined;
