...
export default function PortfolioCardSection({ data, embedded = false }) {
  const stats = data?.stats || [];

  const card = (
    <div className="w-full bg-white border border-black/10 shadow-md rounded-2xl overflow-hidden animate-fadeInUp">
  ...
  // Rendered inside a shared grid by SectionRenderer when multiple portfolio
  // cards are adjacent — fills its grid cell, no section wrapper of its own.
  if (embedded) {
    return (
      <>
        {card}
